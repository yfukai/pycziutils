...
def parse_filename(filename):
    properties=[p.split("-") for p in path.basename(filename).replace(".czi","").split("_")]
...
def test_read_images_by_dataframe(czi_files_path):
    for name,data in czi_files_path:
        ...
        assert_indices(tiled_properties_dataframe["image"],
                       data["tile"][0]*data["tile"][1])
        assert_indices(tiled_properties_dataframe["C_index"],
                       len(data["channel"]))
        assert_indices(tiled_properties_dataframe["T_index"],
                       data["time"])
        assert_indices(tiled_properties_dataframe["Z_index"],
                       data["z"])

        reader=pycziutils.get_tiled_reader(name) #returns bioformats reader for tiled images
